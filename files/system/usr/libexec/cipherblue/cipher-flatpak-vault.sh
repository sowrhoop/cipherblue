...
# ==============================================================================
# CRITICAL ARCHITECTURE FIX: DYNAMIC OVERRIDE RECONCILIATION
# ==============================================================================
...
if [ -d "/etc/cipherblue/flatpak-overrides" ] && [ "$(ls -A /etc/cipherblue/flatpak-overrides 2>/dev/null)" ]; then
    ...
    # DYNAMIC INJECTION: Strictly target the Single-Tenant System Owner (UID 1000)
    PRIMARY_USER=$(awk -F: '$3 == 1000 {print $1; exit}' /etc/passwd)
    if [[ -n "$PRIMARY_USER" ]]; then
        cipher_log "Injecting dynamic OS user mappings for: $PRIMARY_USER"
        find /var/lib/flatpak/overrides/ -type f -exec sed -i "s/CIPHERBLUE_USER/$PRIMARY_USER/g" {} +
    fi
    changes_made=true
fi
